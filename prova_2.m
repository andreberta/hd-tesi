...
for ii=1:length(regions_lh)
   ...
   %extract parc info
    ...
    mask = parc_lh_img== pos;
    %patch number
    patch_number = sum(mask(:));
...
%         S = S(:,mask_lh);
        [S,~] = random_patches_2(img,psz,patch_number,~mask);
        % remove black patches
        mean_S = mean(S,1);
        S = S(:,mean_S > 0);
        %remove mean from patches
        S = bsxfun(@minus,S,mean_S);
        %store them
        whole_patches(:,curr:curr+patch_number-1) = S;
        curr = curr+patch_number;
    end
    clear curr S img v_curv_lh visit_path mask parc_lh_img
